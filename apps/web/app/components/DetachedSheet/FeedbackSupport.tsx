...
const FeedbackContent = () => {
  ...
  return (
    <div className="!text-white !bg-[#171717] p-6 rounded-xl space-y-4">
      <Sheet.Title className="ExampleDetachedSheet-title text-center">
        Feedback
      </Sheet.Title>
      <Sheet.Description className="!text-slate-400 text-center">
      ...
      <Tabs value={feedbackType} onValueChange={setFeedbackType} className="w-full !rounded-md">
        <TabsList className="w-full !rounded-md">
          {["Issue", "Idea", "Other", "Compliment"].map((type) => (
            <TabsTrigger 
              value={type}
              className={`!rounded-md w-full ${typeActiveStyle[type]}`}
              key={type}
            >
              {type}
            </TabsTrigger>
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            readOnly
            className="w-full px-3 py-2 !bg-[#2a2a2a] !text-white rounded-md !border !border-[#3a3a3a] focus:!outline-none focus:!border-blue-500"
          />
        </div>
        ...
        {submitStatus === "success" && (
          <p className="text-green-500 text-sm">Your feedback has been submitted successfully!</p>
        )}
        ...
        <button
            className="ExampleDetachedSheet-validateTrigger w-fit flex items-center justify-center cursor-pointer"
...
const SupportContent = () => {
  return (
    <div className="!text-white !bg-[#171717] p-6 rounded-xl space-y-6">
      <Sheet.Title className="ExampleDetachedSheet-title">Support</Sheet.Title>

      <Sheet.Description className="!text-slate-400 text-center">
        Our support team is here to assist you with any questions or issues you may have.
      </Sheet.Description>

      <div className="!bg-[#1f1f1f] !p-5 !rounded-lg !border !border-[#2a2a2a]">
        ...
        <div className="flex gap-2 flex-wrap">
          <a
            ...
            className="flex items-center gap-1 !bg-[#2a2a2a] hover:!bg-[#333] !text-white px-3 py-2 rounded-md text-sm transition-colors"
          >
      ...
      <div className="flex w-full items-center justify-center flex-wrap gap-3">
        <a
          href="/docs"
          className="!bg-[#2a2a2a] !text-white hover:!bg-[#333] px-4 py-2 rounded-md text-sm"
        >
          View Docs
        </a>
        <a
